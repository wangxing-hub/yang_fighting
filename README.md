...
首屏总共约 **1.7MB**（Phaser 1.17MB + 人物贴图 0.5MB + 头像 57KB；
**街道背景是代码画的，完全不占下载**）。GitHub Pages 会对 JS 自动 gzip，
实际传输大约 **0.8MB**。

加载顺序是特意拆开的：

1. 启动页只下**三个头像（约 60KB）**，所以标题页几乎立刻就能出来；
2. 人物精灵表在标题页**后台下载**，进度直接显示在「开始挑战」按钮上
   （没下完就点，会变成"素材准备中…"，下完自动进关）。

图片是怎么瘦下来的：

| 做法 | 效果 |
| --- | --- |
| 人物贴图从**无损** webp 改成**有损 q86**（`-alpha_q 100`，alpha 通道无损） | 2.15MB → 0.44MB，平均色差 1.9/255、最大单通道差 17，边缘 alpha 一个像素都没变 |
| 删掉三张已经用不到的图（杨的旧投掷、黄姐的旧便服走路、松鸭湖背景） | 少下 0.3MB |
| 街道背景改成代码画 | 少下 0.09MB |

还想更小：把 `SMALL_JOBS` 里的 `scale` 从 `0.7` 调到 `0.6`、
或者把 `quality` 从 `86` 降到 `80`，再跑 `npm run assets small`。
